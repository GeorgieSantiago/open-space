...
function toNumber(value) {
  ...
  val = value.replace(reTrim, '');
  const isBinary = reIsBinary.test(val);
  if (isBinary || reIsOctal.test(val)) {
    // -> No nested ternaries.
    let parsedInt = null;
    let parseRate = 8;
    if (isBinary) {
    ...
    parsedInt = freeParseInt(val.slice(2), parseRate);
    if (parsedInt) {
      return parsedInt;
    }
  }
  if (reIsBadHex.test(val)) return NAN;
  return +val;
}
...
function debounce(func, wait, options) {
  ...
  function remainingWait(time) {
    ...
    const remaining = waitTime - timeSinceLastCall;

    return maxing
      ? nativeMin(remaining, maxWait - timeSinceLastInvoke)
      : remaining;
  }
  ...
  if (isObject(options)) {
    ...
    maxWait = maxing
      ? nativeMax(toNumber(options.maxWait) || 0, waitTime)
      : maxWait;
